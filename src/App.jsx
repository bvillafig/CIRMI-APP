...
async function dbGet(table, params = "") {
  const res = await fetch(`${API(table)}?${params}`, { headers: HEADERS });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
async function dbInsert(table, data) {
  const res = await fetch(API(table), { method: "POST", headers: HEADERS, body: JSON.stringify(data) });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
async function dbUpdate(table, id, data) {
  const res = await fetch(`${API(table)}?id=eq.${id}`, { method: "PATCH", headers: HEADERS, body: JSON.stringify(data) });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
async function dbDelete(table, id) {
  const res = await fetch(`${API(table)}?id=eq.${id}`, { method: "DELETE", headers: HEADERS });
  if (!res.ok) throw new Error(await res.text());
}
...
const PERSONAL_DEFAULT = [
  { id: 1, nombre: "Dr. García", rol: "Cirujano Principal", hospitales: ["Hospital A","Hospital B","Hospital C"], tel: "655 000 000", color: B.slate },
  { id: 2, nombre: "Dr. Ruiz", rol: "Cirujano Residente", hospitales: ["Hospital A","Hospital B"], tel: "655 000 001", color: B.slateDark },
  { id: 3, nombre: "Dr. López", rol: "Cirujano Residente", hospitales: ["Hospital B","Hospital C"], tel: "655 000 002", color: B.slateLight },
  { id: 4, nombre: "Enf. Martínez", rol: "Enf. Instrumentista", hospitales: ["Hospital A","Hospital C"], tel: "655 000 003", color: B.goldDark },
...
export default function App() {
  ...
  const [personal] = useState(PERSONAL_DEFAULT);
  ...
  // ── Cargar cirugías desde Supabase ──
  const loadCirugias = async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await dbGet("cirugias", "order=fecha.desc,inicio.asc");
      setCirugias(data);
    } catch (e) {
  ...
  useEffect(() => { loadCirugias(); }, []);

  const openNew = () => {
    setForm({ id: newId(), fecha: selectedDate, hospital: "Hospital A", quirofano: "Q-1", tipo: "", cirujano: "Dr. García", ayudante: "", enfermera: "", inicio: "08:00", fin: "10:00", estado: "Confirmada", factura: "Pendiente", paciente: "", obs: "" });
    setModal("nueva");
  };
  ...
  const saveForm = async () => {
    ...
    try {
      if (modal === "nueva") {
        await dbInsert("cirugias", form);
      } else {
        const { id, ...data } = form;
        await dbUpdate("cirugias", id, data);
      }
      await loadCirugias();
      setModal(null);
    } catch (e) {
  ...
  const updateFactura = async (id, value) => {
    try {
      await dbUpdate("cirugias", id, { factura: value });
      setCirugias(prev => prev.map(c => c.id === id ? { ...c, factura: value } : c));
    } catch (e) {
    ...
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: B.bg, minHeight: "100vh", color: B.text }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-sec{background:white;color:${B.slate};border:1.5px solid ${B.border};padding:9px 18px;border-radius:9px;font-size:14px;font-weight:500;transition:all .15s}
        .btn-sec:hover{border-color:${B.slateLight};background:#F8FAFB}
        .btn-danger{background:#FEF2F2;color:#DC2626;border:1.5px solid #FECACA;padding:9px 18px;border-radius:9px;font-size:14px;font-weight:500}
        .btn-danger:hover{background:#FEE2E2}
        .tab-btn{padding:9px 18px;border:none;background:none;font-size:14px;font-weight:500;color:rgba(255,255,255,.6);border-radius:8px;transition:all .15s}
        ...
        .inp{width:100%;padding:9px 12px;border:1.5px solid ${B.border};border-radius:8px;font-size:14px;outline:none;transition:border .15s;color:${B.text}}
        .inp:focus{border-color:${B.slate}}
        .row-card{background:white;border:1.5px solid ${B.border};border-radius:11px;padding:14px 18px;margin-bottom:8px;cursor:pointer;transition:all .15s;display:grid;grid-template-columns:80px 1fr 1fr 1fr 100px 100px 28px;gap:12px;align-items:center}
        ...
        .modal{background:white;border-radius:18px;padding:32px;max-width:640px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 30px 80px rgba(30,43,58,.25)}
        .form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
        .form-group label{display:block;font-size:11px;font-weight:700;color:${B.muted};text-transform:uppercase;letter-spacing:.6px;margin-bottom:6px}
        ...
        .col-header{font-size:11px;font-weight:700;color:${B.muted};text-transform:uppercase;letter-spacing:.5px}
        .spinner{width:36px;height:36px;border:3px solid ${B.border};border-top-color:${B.slate};border-radius:50%;animation:spin 1s linear infinite}
      ...
      <div style={{ background: B.slateDark, padding: "0 28px", position: "sticky", top: 0, zIndex: 50, boxShadow: "0 2px 16px rgba(20,30,42,.35)" }}>
        <div style={{ maxWidth: 1120, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 62 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <button className="btn-gold" onClick={openNew} style={{ padding:"8px 16px", fontSize:13 }}>+ Nueva cirugía</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1120, margin:"0 auto", padding:"28px" }}>
        ...
        {/* ERROR */}
        {error && (
          <div style={{ background:"#FEF2F2", border:"1.5px solid #FECACA", borderRadius:12, padding:"16px 20px", marginBottom:20, display:"flex", alignItems:"center", gap:12 }}>
            <span style={{ fontSize:20 }}>⚠️</span>
            <div>
              <div style={{ fontWeight:600, color:"#B91C1C" }}>Error de conexión</div>
              <div style={{ fontSize:13, color:"#DC2626", marginTop:2 }}>{error}</div>
            </div>
            <button className="btn-sec" onClick={loadCirugias} style={{ marginLeft:"auto", fontSize:13 }}>Reintentar</button>
          </div>
        )}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:28 }}>
          {[
            ...
            { label:"Próximos 7 días", value:cirugias.filter(c=>{const d=new Date(c.fecha);return d>=today&&d<=new Date(today.getTime()+7*86400000);}).length, icon:"📅", accent:B.slateLight },
            { label:"Fact. pendiente", value:pendFactura, icon:"📋", accent:B.goldDark },
            { label:"Total intervenciones", value:cirugias.length, icon:"📊", accent:B.slateDark },
          ].map(s=>(
        ...
        {loading ? (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"80px 0", gap:16 }}>
            ...
            <div style={{ color:B.muted, fontSize:14 }}>Cargando datos de CIRMI...</div>
          </div>
        ) : (
          <>
            {/* ══ AGENDA ══ */}
            {tab==="agenda" && (
              <div>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:12 }}>
                  <div>
                    <h2 style={{ fontSize:22, fontWeight:700, color:B.slateDark }}>Agenda de Quirófano</h2>
                    <p style={{ color:B.muted, fontSize:13, marginTop:3 }}>Intervenciones programadas</p>
                  </div>
                ...
                {cirugiasDia.length===0 ? (
                  <div className="card" style={{ padding:52, textAlign:"center", color:B.muted }}>
                    ...
                    <button className="btn-gold" onClick={openNew} style={{ marginTop:18 }}>+ Añadir cirugía</button>
                  </div>
                  ...
                  <div key={c.id} className="row-card" onClick={()=>{setForm({...c});setModal("edit");}}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:11, color:B.muted, marginTop:2, fontFamily:"'DM Mono',monospace" }}>{c.id}</div>
                      {c.obs && <div style={{ fontSize:11, color:B.goldDark, marginTop:3 }}>⚠ {c.obs}</div>}
                    </div>
              ...
              <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:18 }}>
                  {HOSPITALES.map((h,idx)=>{
                    ...
                    const accents=[B.slate,B.slateDark,B.slateLight];
                    const accent=accents[idx];
                    return (
                      <div key={h} style={{ background:"white", borderRadius:14, overflow:"hidden", boxShadow:"0 1px 4px rgba(46,63,82,.07)" }}>
                        ...
                        <div style={{ padding:"16px 18px" }}>
                          {cxs.length===0 ? (
                            ...
                            <div key={c.id} onClick={()=>{setForm({...c});setModal("edit");}}
                        ...
                        <div style={{ padding:"0 18px 18px", borderTop:`1px solid ${B.border}`, paddingTop:14 }}>
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            {PERSONAL_DEFAULT.filter(p=>p.hospitales.includes(h)).map(p=>(
                              <div key={p.id} style={{ display:"flex", alignItems:"center", gap:6, background:B.bg, borderRadius:20, padding:"4px 10px 4px 5px", border:`1px solid ${B.border}` }}>
                                <div className="avatar" style={{ background:p.color, width:22, height:22, fontSize:9 }}>{p.nombre.split(" ").slice(-1)[0][0]}</div>
                      ...
                      <div key={p.id} className="card" style={{ padding:22, borderLeft:`4px solid ${p.color}` }}>
                        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:16 }}>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:12, color:B.muted, marginTop:2 }}>📞 {p.tel}</div>
                          </div>
                          <div style={{ textAlign:"right" }}>
                            <div style={{ fontSize:26, fontWeight:700, color:p.color }}>{cxs.length}</div>
                            <div style={{ fontSize:11, color:B.muted }}>cirugías</div>
                          </div>
                        </div>
              ...
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:12 }}>
                  <div>
                    ...
                    <p style={{ color:B.muted, fontSize:13, marginTop:3 }}>Seguimiento de cobros por intervención</p>
                  </div>
                ...
                <div className="card" style={{ overflow:"hidden" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"90px 100px 1fr 140px 110px 130px 40px", gap:12, padding:"12px 20px", background:B.bg, borderBottom:`1px solid ${B.border}` }}>
                    {["ID","Fecha","Intervención","Hospital","Estado","Factura",""].map(h=>(
                      <div key={h} className="col-header">{h}</div>
                    ))}
                  </div>
        ...
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
              <div>
                <h3 style={{ fontSize:20, fontWeight:700, color:B.slateDark }}>{modal==="nueva"?"Nueva intervención":"Editar intervención"}</h3>
            ...
            <div className="form-grid">
              {[
                ...
                ["Cirujano principal", <select className="inp" value={form.cirujano||""} onChange={e=>setForm({...form,cirujano:e.target.value})}>{PERSONAL_DEFAULT.map(p=><option key={p.id}>{p.nombre}</option>)}</select>],
                ["Ayudante", <select className="inp" value={form.ayudante||""} onChange={e=>setForm({...form,ayudante:e.target.value})}><option value="">— Sin ayudante —</option>{PERSONAL_DEFAULT.map(p=><option key={p.id}>{p.nombre}</option>)}</select>],
                ["Enfermera instrumentista", <select className="inp" value={form.enfermera||""} onChange={e=>setForm({...form,enfermera:e.target.value})}><option value="">— Sin asignar —</option>{PERSONAL_DEFAULT.filter(p=>p.rol.includes("Enf")).map(p=><option key={p.id}>{p.nombre}</option>)}</select>],
                ...
                <div className="form-group" key={label}><label>{label}</label>{field}</div>
              ))}
            </div>
            ...
            <div style={{ display:"flex", gap:10, marginTop:26, justifyContent:"space-between", alignItems:"center" }}>
              <div>{modal==="edit"&&<button className="btn-danger" onClick={()=>deleteCirugia(form.id)} disabled={saving}>🗑 Eliminar</button>}</div>
              <div style={{ display:"flex", gap:10 }}>
                <button className="btn-sec" onClick={()=>setModal(null)}>Cancelar</button>
                <button className="btn-gold" onClick={saveForm} disabled={saving}>
                  {saving?"Guardando...":modal==="nueva"?"Crear intervención":"Guardar cambios"}
                </button>
              </div>
            </div>
          </div>
